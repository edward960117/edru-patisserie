  ...
  return (
    <section className="grid gap-8 lg:grid-cols-2">
      <div className="card-lux overflow-hidden">
        <div className="relative min-h-[360px] h-full w-full">
          <Image
            src={cake.image_url}
            alt={cakeName}
            fill
      ...
      <article className="card-lux p-6 sm:p-8">
        <p className="lux-kicker">Artisan Selection</p>
        <h1 className="heading-serif mt-2 text-4xl sm:text-5xl leading-tight">{cakeName}</h1>
          ...
          {cake.sizes.map((size) => (
            <li key={size.id} className="rounded-xl border border-[color:var(--gold)]/22 px-4 py-3 bg-[rgba(255,250,241,0.72)]">
              <div className="flex items-center justify-between gap-3">
                ...
                <Link href={`/checkout?cake=${cake.slug}&size=${size.id}`} className="btn-lux text-xs">
                  {copy.checkout}
                </Link>
              </div>
            </li>
