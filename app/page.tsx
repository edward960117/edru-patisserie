...
export default async function HomePage() {
  ...
  return (
    <section className="space-y-7 sm:space-y-10">
      <div className="card-lux hero-lux atelier-frame relative overflow-hidden px-5 py-7 sm:px-10 sm:py-12">
        <div className="absolute -top-14 -right-10 h-40 w-40 rounded-full bg-[#d9b582]/18 blur-2xl" />
        <p className="lux-kicker section-label">{copy.homeTagline}</p>
        <h1 className="heading-serif mt-2 text-[1.86rem] sm:text-6xl leading-[1.05]">{copy.homeTitle}</h1>
        <p className="mt-3 sm:mt-4 max-w-2xl text-[color:var(--ink-faint)] text-[0.98rem] sm:text-lg leading-relaxed">{copy.homeSubtitle}</p>
      </div>
      ...
      <div className="grid home-category-grid gap-3 sm:gap-6">
        {categories.map((category) => (
          ...
          >
            ...
            <p className="mt-auto pt-5 sm:pt-7 text-[0.74rem] sm:text-xs text-[color:var(--gold-deep)] uppercase tracking-[0.15em] sm:tracking-[0.2em] group-hover:tracking-[0.21em]">{copy.viewCategory}</p>
          </Link>
        ))}
      </div>
    </section>
